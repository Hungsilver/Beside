...
| F12 | Học cùng nhau | ✅ Phòng Pomodoro chung: **đồng hồ số lật** chạy đồng bộ hai máy, đặt tên môn cho buổi học, mục tiêu phút mỗi ngày, thống kê 7 ngày + phân bổ theo môn + chuỗi ngày, mốc đếm ngược, việc cần làm, nhật ký một dòng mỗi chặng, dải thời gian trong ngày, so kè tuần, tiếng ồn nền tổng hợp, cổ vũ thời gian thực. **Không có gọi video** — xem §1.1b. Đồng hồ toàn màn hình đã tách sang **F15** |
| F15 | Đồng hồ | ✅ **Module riêng** ở `/dong-ho`, ngang hàng F12. Đồng hồ số lật toàn màn hình, **một thẻ cho mỗi đơn vị** (`09`) như đồng hồ lật thật; 4 chế độ (giờ hiện tại · hẹn giờ · bấm giờ có ghi vòng · phiên học khi đang có phiên), 4 chủ đề màu, ngày/thứ, mức pin, hẹn giờ gõ số phút tuỳ ý, chỉnh sáng/tiếng bằng vuốt, giữ màn hình sáng, tự tối ban đêm, chống lưu ảnh, xoay ngang. **Không có báo thức và không có thời tiết** — xem ADR §10 |
...
| **7b** | ✅ **F15 Đồng hồ tách thành module riêng** (`/dong-ho`) — bố cục lại theo dáng app Flip Clock: mặt số chiếm ~72% chiều cao, đáy chỉ một hàng tab, cài đặt vào bảng trượt | 4 E2E (DH-01..04) · 14 unit test |
...
| 2026-09-12 | **F12**: KHÔNG làm báo thức, dù đó là tính năng số một của app đồng hồ | PWA không có cách nào đánh thức khi app đã đóng. Web Push tới được nhưng hệ điều hành quyết định có kêu hay không và kêu bao lâu — hứa một cái chuông rồi nó không kêu thì tệ hơn hẳn là không có. Hẹn giờ tự do thì có làm, và nói thẳng trên màn hình rằng chuông chỉ kêu khi app còn mở | Muốn báo thức thì dùng app đồng hồ của máy |
| 2026-09-12 | **F12**: KHÔNG làm widget thời tiết như các app Flip Clock khác | Cần một dịch vụ ngoài + khoá API, mà §4 chốt danh sách công nghệ và §3 chốt bản đồ dùng nguồn không cần khoá. Thêm một phụ thuộc ngoài cho một dòng nhiệt độ là không đáng | Thiếu một thứ trang trí quen thuộc |
| 2026-09-12 | **F12**: chủ đề màu đi qua BIẾN CSS, không qua class trên component | `.flip-digit` nằm sâu trong `FlipClock` và còn được dùng ở thẻ hồng của phòng học. Biến thừa kế xuống là chỗ duy nhất đổi được màu mà không phải sửa component hay truyền prop qua ba tầng | Chủ đề nào nền SÁNG (`Giấy`) thì mọi chữ trắng phải lật sang đen — giao diện hỏi `isLightTheme()` |
| 2026-09-12 | **F12**: chống lưu ảnh đi theo hình thoi 4 đỉnh, không nhảy ngẫu nhiên | Ngẫu nhiên có thể rơi hai lần liền vào cùng một chỗ, mà mục đích là điểm ảnh nào cũng được nghỉ | Dịch 6px mỗi phút — mắt không thấy, nhưng ai soi kỹ sẽ nhận ra đồng hồ hơi lệch tâm |
| 2026-09-12 | **F12**: chế độ đêm chỉ ĐẶT SÀN độ tối, không ghi đè lựa chọn người dùng | Ghi đè thì ai vừa kéo sáng lên sẽ thấy màn hình tự tối lại sau một nhịp — trông như app đang cãi lại mình | Ban đêm kéo sáng hết cỡ vẫn chỉ tới mức sàn nếu không tắt chế độ đêm |
| 2026-09-12 | **F15**: đồng hồ toàn màn hình TÁCH khỏi Phòng học thành module riêng `/dong-ho` | Nó là lớp phủ bên trong `StudyScreen` và phải được màn đó truyền `session` + `noise` vào, trong khi thứ dùng thường xuyên nhất — dựng máy trên bàn xem giờ — chẳng cần phiên học nào. Chủ dự án chốt: một module ngang hàng, và Phòng học chỉ còn một đường dẫn sang | Tiếng ồn nền KHÔNG còn kêu tiếp khi đi từ Phòng học sang đồng hồ: hai màn là hai `useNoise()` khác nhau, rời màn là tắt tiếng. Muốn giữ thì phải nâng `AudioContext` lên context dùng chung cho cả app |
| 2026-09-12 | **F15**: mỗi ĐƠN VỊ một thẻ (`09`), không phải mỗi chữ số một thẻ | Đó là dáng của đồng hồ lật thật và của các app Flip Clock — một lá thẻ in sẵn hai chữ số. Quan trọng hơn: một thẻ `41` rộng 1.46em còn hai thẻ `4` `1` rời rộng 2.05em, nên cỡ chữ tính ngược từ bề ngang khả dụng lớn hơn ~40% ở cùng một khung | `FlipClock` phải mang hai kiểu thẻ (`card="digit"` cho thẻ hồng của Phòng học, `card="group"` cho module Đồng hồ) và `fluidStyle()` phải tính riêng cho từng kiểu |
| 2026-09-12 | **F15**: đáy màn chỉ còn MỘT hàng tab; mọi cài đặt vào bảng trượt lên | Bản trước bày bốn chip chế độ + một hàng nút riêng + nút cài đặt + một dòng hướng dẫn; cộng lại chúng ăn hơn một phần ba chiều cao màn hình, mà thứ người ta mở app lên để xem là mặt số. Giờ mặt số chiếm ~72% chiều cao ở khổ 390×844, chốt bằng E2E `DH-01` | Đổi chủ đề hay bật/tắt giây mất thêm một cú chạm để mở bảng |
| 2026-09-12 | **F15**: chế độ hẹn giờ khi CHƯA đặt gì thì thay hẳn mặt số bằng màn đặt | Một mặt số đang hiện `00:00` không nói lên điều gì, nên chỗ của nó lúc đó dùng cho việc đang làm thì đúng hơn — các app Flip Clock cũng vậy. Nhờ đó ô gõ số phút tuỳ ý có chỗ đứng đàng hoàng, đóng luôn mục BACKLOG 12/09 | Khung giữa lúc này KHÔNG dùng `container-type: size` nữa (nó phớt lờ chiều cao nội dung) mà chuyển sang cuộn dọc thường |
| 2026-09-12 | **F15**: `/dong-ho` KHÔNG bọc trong `RequireCouple`; chế độ "phiên học" chọn bằng `?che-do=phien-hoc` | Xem giờ không cần người thứ hai, nên đồng hồ phải mở được cả khi chưa ghép đôi — `useStudySummary({ enabled })` tự tắt truy vấn lúc đó. Còn chế độ mở sẵn thì không để đồng hồ tự đoán: vào từ trang chủ là muốn XEM GIỜ, vào từ Phòng học lúc đang học là muốn thấy chặng đếm ngược; hai ý đó chỉ phân biệt được bằng đường vào | Thêm một tham số truy vấn vào bề mặt URL; và `useStudySummary` phải nhận thêm tuỳ chọn |
| 2026-09-12 | **F15**: lớp nút khi đã mờ thì `pointer-events-none` | Hàng tab chiếm hết bề ngang màn hình, nên cú chạm "để gọi nút hiện lại" gần như chạm đâu cũng trúng một tab — người dùng đổi chế độ mà không hề thấy mình bấm vào đâu. Giờ cú chạm đầu rơi xuống lớp gốc và chỉ gọi nút hiện lại, đúng kiểu trình phát video | Phải chạm hai lần khi các nút đang mờ (chạm gọi hiện, chạm nữa mới bấm) |
| 2026-09-12 | **F15**: có hiện mức pin, dù Battery Status API không phổ biến | Máy dựng trên bàn hàng giờ với màn hình bị giữ sáng thì "còn bao nhiêu pin" là thông tin đúng lúc nhất có thể có trên màn hình đó — và mọi app đồng hồ để bàn đều hiện. Chỉ Chrome/Edge trả lời; Safari không có, Firefox đã gỡ hẳn vì API này giúp nhận dạng máy | Mặc định TẮT, và khi trình duyệt không trả lời thì phần này tự ẩn chứ không hiện "—%" |
