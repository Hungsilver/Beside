...
| 09/09 | **Tiến lên: luật ăn tiền chưa làm** — tới trắng (6 đôi, sảnh rồng, tứ quý heo), thối bài/đền khi ôm heo, phạt theo số lá còn lại. Chủ dự án chốt chỉ làm luật cơ bản + chặt heo | Chờ quyết định | `docs/thiet-ke-games.md` §1 |
...
| 09/09 | `hasAnswer()` chỉ dò bộ cùng loại + hàng chặt, không liệt kê toàn bộ tổ hợp 13 lá — về lý thuyết có thể bỏ sót một nước rất hiếm và báo "bí" oan. Nút Bỏ lượt vẫn luôn ở đó nên không chặn ai | Thấp | `packages/shared/src/tien-len.ts` |
...
| ~~09/09~~ | ~~Chưa có trace API cho Phase 6~~ → **ĐÃ LÀM** cùng ngày: `apps/api/test/trace-phase6-games.mjs`, **45/45 case** qua REST + WebSocket thật, đã nối vào `verify:local` | ~~Trung bình~~ | — |
