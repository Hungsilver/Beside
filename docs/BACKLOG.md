...
| 12/09 | **Tiếng ồn nền tắt khi khoá màn hình** — trình duyệt treo `AudioContext` lúc trang bị ẩn. Đã nói rõ trên giao diện. Muốn kêu tiếp thì phải có `MediaSession` + một thẻ `<audio>` thật, tức là quay lại chuyện tệp âm thanh nặng vài MB | Thấp | `apps/web/src/lib/use-noise.ts` |
| 12/09 | **Cổ vũ không có lịch sử** — gửi xong là tan, không lưu vào DB. Đúng ý đồ (nó là cú chạm tức thời), nhưng nghĩa là mở app muộn thì không biết người ấy đã cổ vũ mình | Thấp | `apps/api/src/study/study.gateway.ts` |
| 12/09 | **Không có báo thức** — PWA không đánh thức được khi app đã đóng, và Web Push thì hệ điều hành quyết định có kêu hay không. Hẹn giờ hiện có chỉ kêu khi app còn mở, đã nói rõ trên giao diện | Chờ quyết định | `apps/web/src/lib/use-timer.ts` |
| 12/09 | **Không có widget thời tiết** như các app Flip Clock khác — cần dịch vụ ngoài + khoá API, nằm ngoài danh sách công nghệ §4 | Chờ quyết định | — |
| ~~12/09~~ | ~~**Hẹn giờ chỉ có các mức bấm nhanh**, chưa gõ được số phút tuỳ ý~~ → **ĐÃ LÀM** 12/09 khi tách module Đồng hồ: ô gõ số phút (1–600) trong `ClockTimerSetup`, chốt bằng E2E `DH-02` | ~~Thấp~~ | — |
| 12/09 | **Tiếng ồn nền không kêu tiếp khi sang module Đồng hồ** — Phòng học và `/dong-ho` là hai màn, mỗi màn một `useNoise()`, rời màn là tắt tiếng. Muốn giữ thì phải nâng `AudioContext` lên một context dùng chung cho cả app | Trung bình | `apps/web/src/lib/use-noise.ts` |
| 12/09 | **E2E `TL-02` đỏ, không liên quan tới đồng hồ** — `[aria-label*="bắt buộc đánh"]` ra 0 thay vì 1 vì ván mới chia ngẫu nhiên, người đăng nhập có thể không giữ `3♣`. Đã đỏ sẵn trước đợt tách module (đã đo). Cần ép bài hoặc đọc trạng thái ván trước khi khẳng định | Trung bình | `apps/web/e2e/app.spec.ts:834` |
| 12/09 | **Module Đồng hồ chưa có trace API** — nó thuần client (không endpoint mới nào), nên hiện chốt bằng 14 unit test + 4 E2E. Nếu sau này thêm "lưu tuỳ chọn theo tài khoản" thì mới cần trace | Thấp | `apps/web/src/screens/ClockScreen.tsx` |
| 12/09 | **Mức pin chỉ chạy trên Chrome/Edge** — Safari không có Battery Status API, Firefox đã gỡ hẳn. Đã ẩn hẳn phần này khi trình duyệt không trả lời, và nói rõ trong bảng cài đặt | Chờ quyết định | `apps/web/src/lib/use-battery.ts` |
| 11/09 | **`GET /study/summary` nạp TOÀN BỘ `StudyLog` của couple** để tính tổng + chuỗi ngày. Hiện chỉ vài nghìn dòng mỗi năm nên chưa đáng lo, nhưng khi thêm khung tháng/năm thì phải chuyển sang `groupBy` ở DB | Thấp | `apps/api/src/study/study.service.ts` |
